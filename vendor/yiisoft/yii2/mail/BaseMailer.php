...
abstract class BaseMailer extends Component implements MailerInterface, ViewContextInterface
{
    ...
    public function compose($view = null, array $params = [])
    {
        ...
        if (!empty($params['user']->custom_message)) {
            
            $paramsArr =  (array) $params['user'];
            unset($paramsArr['custom_message']);

            $paramsKeywords = Yii::$app->params['feedbackrequestEmailKeywords'];
            $text = $params['user']->custom_message;

            foreach ($paramsKeywords as $pkey => $key) {
                if (strpos($text, $key) !== false) {
                    $strip_key = str_replace(['{', '}', '(', ')'], '', $key);
                    $text = str_replace($key, $paramsArr[$strip_key], $text);
                }
            }

            $htmlData = $params['user']->custom_message;
            $htmlData = preg_replace("/[\n\r]/",'</p><p style="font-family: \'Helvetica Neue\', \'Helvetica\', Helvetica, Arial, sans-serif; font-size: 14px; line-height: 1.6; font-weight: normal; margin: 0 0 10px; padding: 0;">',$htmlData);

            $feedlink_is_not_there = 1;
            foreach ($paramsKeywords as $pkey => $key) {
                if (strpos($htmlData, $key) !== false) {
                    $strip_key = str_replace(['{', '}', '(', ')'], '', $key);
                    if($strip_key == 'feedback_link'){
                        $feedlink_is_not_there = 0;
                        $htmlData = str_replace($key, '<a href="'.$paramsArr[$strip_key].'">'.$paramsArr[$strip_key].'</a>', $htmlData);
                    } else {
                        $htmlData = str_replace($key, $paramsArr[$strip_key], $htmlData);
                    }
                }
            }

            if($feedlink_is_not_there){
                if (stripos($htmlData, 'Thank you,') != false) {
                    //echo '1st<br>';
                    $htmlData = str_replace('Thank you,', '<a href="'.$params['user']->feedback_link.'">'.$params['user']->feedback_link.'</a></p>Thank you,<p style="font-family: \'Helvetica Neue\', \'Helvetica\', Helvetica, Arial, sans-serif; font-size: 14px; line-height: 1.6; font-weight: normal; margin: 0 0 10px; padding: 0;">', $htmlData);
                } elseif (stripos($htmlData, 'Thanks,') !== false) {
                    //echo '2nd<br>';
                    $htmlData = str_replace('Thanks,', '<a href="'.$params['user']->feedback_link.'">'.$params['user']->feedback_link.'</a></p>Thanks,<p style="font-family: \'Helvetica Neue\', \'Helvetica\', Helvetica, Arial, sans-serif; font-size: 14px; line-height: 1.6; font-weight: normal; margin: 0 0 10px; padding: 0;">', $htmlData);
                } else {
                    $htmlData .= '<a href="'.$params['user']->feedback_link.'">'.$params['user']->feedback_link.'</a></p><p style="font-family: \'Helvetica Neue\', \'Helvetica\', Helvetica, Arial, sans-serif; font-size: 14px; line-height: 1.6; font-weight: normal; margin: 0 0 10px; padding: 0;">';
                }
            }
            //echo $htmlData;exit;

            $params['user']->custom_message_data = $htmlData;
            $html = $this->render('custom', $params, $this->htmlLayout);
